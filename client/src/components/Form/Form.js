  ...
  return (
    <Paper className={classes.paper}>
      <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        <Typography className={classes.formTitle} variant="h6">{currentId ? `Editing "${post.title}"` : 'Import an Image'}</Typography>
        <TextField name="title" variant="outlined" label="Image Title" fullWidth value={postData.title} onChange={(e) => setPostData({ ...postData, title: e.target.value })} />
